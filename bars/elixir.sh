...
bl_bar_init elixir "#e8d8f8" "#a078d8" '["#180c2e","#2a1850"]' "$SIGNAL_DIR/mix.exs" "$SIGNAL_DIR/mix.lock"

[[ ! -f "$SIGNAL_DIR/mix.exs" ]] && exit 0
...
elixir_version=$(grep -m1 'elixir:' "$SIGNAL_DIR/mix.exs" 2>/dev/null \
  | grep -oE '[0-9]+\.[0-9]+(\.[0-9]+)?')

# Fallback: .tool-versions (asdf/mise) or .elixir-version
if [[ -z "$elixir_version" && -f "$SIGNAL_DIR/.tool-versions" ]]; then
  elixir_version=$(awk '/^elixir /{print $2; exit}' "$SIGNAL_DIR/.tool-versions" 2>/dev/null)
fi
[[ -z "$elixir_version" && -f "$SIGNAL_DIR/.elixir-version" ]] && \
  elixir_version=$(tr -d '[:space:]' < "$SIGNAL_DIR/.elixir-version")

# ── Detect Phoenix from mix.lock ──────────────────────────────────────────────
has_phoenix=false phoenix_version=''
lock="$SIGNAL_DIR/mix.lock"
if [[ -f "$lock" ]]; then
...
if ! $has_phoenix && grep -q ':phoenix,' "$SIGNAL_DIR/mix.exs" 2>/dev/null; then
  has_phoenix=true
fi
...
has_exunit=false
[[ -d "$SIGNAL_DIR/test" ]] && has_exunit=true
