...
| `auto_bars.search_depth` | Integer. Maximum subdirectory depth to search for signal files (default: `0` = root only). Set to `1` or `2` for monorepos whose manifests live below the project root. Heavy directories (`.git`, `node_modules`, `vendor`, `.build`, `target`, `Pods`, `DerivedData`, `dist`, `build`, `.venv`) are always pruned. |
